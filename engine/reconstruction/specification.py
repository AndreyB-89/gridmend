...
def readback(spec):
    dims = ', '.join(f'{"axial height" if k == "height" and spec.family == "ring" else k.replace("_", " ")} {spec.dimensions[k].value_mm:g} mm' for k in required(spec) if spec.dimensions.get(k) and spec.dimensions[k].value_mm is not None)
