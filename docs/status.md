...
| **Devin mode: stub only** (Sun 10:05, user's OK after code freeze). `POST /api/devin/session` builds the Devin prompt from confirmed sizes + observations and starts a session. No key → MOCK, nothing is sent. Not wired to the UI, not demoed. The validator loop (our 5 CAD checks → back to Devin) is **not built** | `engine/providers/devin.py`, `api/main.py`, `tests/test_devin_stub.py` | 3 tests; live call `POST /api/devin/session` → MOCK prompt |
...
- **The app rebuilds rings only.** Three parts are ring-only: `auto_detect` (finds a yellow ring), `fit_ring` (fits two circles) and `cad/ring.py` (the only CAD template). A better detector alone would not change this. The general path is Devin writing CadQuery checked by our validator, and only the prompt half of it exists.
