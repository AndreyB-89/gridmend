...
export function VideoChat({video}: {video: VideoControl}) {
  return <>{video.job?.messages.map((m,i) => <div className={`msg ${m.role === 'user' ? 'me' : 'ai'}`} key={m.id ?? i}><div className="av" aria-hidden="true">{m.role === 'user' ? 'You' : 'AI'}</div><div className="bubble"><p style={{whiteSpace:'pre-wrap'}}>{m.text}</p>{m.role !== 'user' && <ModePill label="Reconstruction" mode={m.mode ?? video.job!.mode}/>}</div></div>)}
...
export function VideoResult({video}: {video: VideoControl}) {
  ...
  return <div className={`viewer ${full ? 'has-model' : ''}`}><div className="v-top"><div><h2>{repair ? 'Proposed missing part' : full ? 'Complete reference' : 'Reconstruction'}</h2><p className="sub">{j?.status.replaceAll('_',' ').toLowerCase() ?? 'Waiting for photo or video'}</p></div>{j && <ModePill label="Reconstruction" mode={j.mode}/>}</div>
