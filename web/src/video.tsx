...
export function VideoResult({video}: {video: VideoControl}) {
  ...
  return <><div className={`viewer ${full ? 'has-model' : ''}`}><div className="v-top"><div><h2>{repair ? 'Missing part' : full ? 'Complete reference' : 'Reconstruction'}</h2><p className="sub">{j?.status.replaceAll('_',' ').toLowerCase() ?? 'Waiting for video'}</p></div>{j && <ModePill label="Video workflow" mode={j.mode}/>}</div>
    ...
    {full && <div className="legend"><span className="pill"><span className="sw y"/>Complete reference</span>{repair && <span className="pill"><span className="sw"/>Validated missing material</span>}</div>}</div>
    ...
    <p className="honest">No physical fit tested. Hidden damage may need more evidence.</p></div></>;
}
