...
export function VideoResult({video}: {video: VideoControl}) {
  ...
  return <><div className={`viewer ${full ? 'has-model' : ''}`}><div className="v-top"><div><h2>Rebuilt {j?.spec.family ?? 'part'}</h2><p className="sub">{j?.status.replaceAll('_',' ').toLowerCase() ?? 'Waiting for video'}</p></div>{j && <ModePill label="Devin" mode={j.mode}/>}</div>
