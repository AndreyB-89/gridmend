...
export function VideoResult({video}: {video: VideoControl}) {
  ...
  return <><div className={`viewer ${full ? 'has-model' : ''}`}><div className="v-top"><div><h2>{repair ? 'Proposed missing part' : full ? 'Complete reference' : 'Reconstruction'}</h2><p className="sub">{repair ? 'Approximate demo' : j?.status.replaceAll('_',' ').toLowerCase() ?? 'Waiting for video'}</p></div>{j && <ModePill label="Video workflow" mode={j.mode}/>}</div>
    ...
    <p className="honest">Approximate demo · 70% silhouette overlap required on two views. Proposed missing material is shown in red. No physical fit tested.</p></div></>;
}
